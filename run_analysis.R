...
loaddata_function <- function(type="test"){
  ...
  ###Determine which columns contains mean or std keywords and assign into "meanstd_features"
  meanstd_features <- grepl("mean|std", features[,2])
  
  ###Selecting only columns where the keywords are TRUE
  x = x[,meanstd_features]
  
  ###Reading the labels details
